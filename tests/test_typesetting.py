...
class test_replace_unicode_characters(unittest.TestCase):
    ...
    def test_that_formulas_are_replaced(self):
        self.assertNotEqual(
            typesetting.replace_unicode_characters('π', True), 'π')
        self.assertNotEqual(
            typesetting.replace_unicode_characters('π', False), 'π')


class test_get_matching_brace(unittest.TestCase):
    def test_closing_brace_found_when_only_one_brace_present(self):
        text = 'text{ok}'
        self.assertEqual(typesetting.get_matching_brace(
            text, 4), len(text) - 1)
        self.assertEqual(typesetting.get_matching_brace(
            text + 'foo', 4), len(text) - 1)
...
class test_escape_unicode_maths(unittest.TestCase):
    ...
    def test_that_flag_to_preserve_alphas_is_passed_through(self):
        res = typesetting.escape_unicode_maths(
            '\\text{ö}', replace_alphabeticals=False)
    ...
    def test_that_all_characters_are_preserved_when_no_replacements_happen(self):
        ...
        self.assertEqual(
            typesetting.escape_unicode_maths(
                text, replace_alphabeticals=False), text
        )
    ...
    def test_that_everything_around_surrounded_character_is_preserved(self):
        ...
        result = typesetting.escape_unicode_maths(
            text, replace_alphabeticals=True)
        ue_pos = text.index('ü')
