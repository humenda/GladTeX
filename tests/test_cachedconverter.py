...
def mk_eqn(eqn, count=0, pos=(1, 1)):
    ...
    Each formula must look like this: (eqn, pos, path, dsp, count) for
    self._convert_concurrently, this is a shorthand with mocking a few
    values.
    """
...
class Tex2imgMock:
    ...
    def convert(self, tx, basename):
        ...
        remove_all(dvi, basename + '.tex', basename +
                   '.log', basename + '.aux')
...
class TestCachedConverter(unittest.TestCase):
    ...
    def test_that_all_converted_formulas_are_in_cache_and_meta_info_correct(self):
        formulas = [mk_eqn('a_{%d}' % i, pos=(i, i), count=i)
                    for i in range(4)]
        c = cachedconverter.CachedConverter('.')
        c.convert_all(formulas)
