...
def exec_setup_py(arg_string):
    """Execute `python setup.py` as a subprocess.

    Use Wine, if necessary.
    """
    ret = None
...
class TemporaryBuildDirectory:
    ...
    Upon entering the context, the source is copied to a temporary
    directory and the program changes to this directory. After all build
    actions have been done, the output file is copied back to the
    original directory, the program resets the current working directory
    ...
    def __exit__(self, _a, _b, _c):
        ...
        shutil.copy(
            os.path.join(
                self.tmpdir, self.output_file_name), self.output_file_name
        )
        shutil.rmtree(self.tmpdir, onerror=self.__onerror)

    def get_temp_directory(self):
        """Find a temporary directory to work in.

        The checks are done to find a directory which does not reside
        within the user's path, because py2exe includes absolute paths
        for python scripts (in their tracebacks). It is not desirable to
        show the whole world the directory layout of the computer where
