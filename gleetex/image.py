...
Each formula is saved as an image, either as PNG or SVG. SVG is advised,
since it is a properly scalable format.
"""
...
DVISVGM_DEPTH_REGEX = re.compile(
    r'^\s*width=.*?pt, height=.*?pt, depth=(.*?)pt')
...
def proc_call(cmd, cwd=None, install_recommends=True):
    """Execute cmd (list of arguments) as a subprocess.

    Returned is a tuple with stdout and stderr, decoded if not None. If
    the return value is not equal 0, a subprocess error is raised.
    Timeouts will happen after 20 seconds.
    """
    with subprocess.Popen(
        ...
        try:
            data = [
                ...
                for d in proc.communicate(timeout=20)
                if d
            ]
            if proc.wait():
...
class Tex2img:
    """Convert a TeX document string into a png file. This class interacts with
    the LaTeX and dvipng/dvisvgm sub processes. Upon error the methods throw a
    SubprocessError with all necessary information to fix the issue.

    On PNG:     The background of the PNG files will be transparent by
    default. If you set a     background colour within the LaTeX
    document, you need to turn off     transparency in this converter
    manually.
    """
    ...
    def set_dpi(self, dpi):
        ...
        This has no effect ifthe output format is SVG. It will
        automatically overwrite a font size, if set.
        """
    ...
    def set_fontsize(self, size):
        """Set font size for formulas.

        This will be automatically translated into a DPI resolution for
        PNG images and taken literally for SVG graphics.
        """
    ...
    def set_transparency(self, flag):
        ...
        This is only relevant for PNG output and if a background colour
        other than "transparent" is required, in this case this set'r
        should be set to false. It is set to True, resulting in a
    ...
    def create_dvi(self, tex_document, dvi_fn):
        ...
        Temporary files will be removed, even in the case of a LaTeX
        error. This method raises a SubprocessError with the helpful
        part of LaTeX's error output.
        """
        ...
        def new_extension(x): return os.path.splitext(dvi_fn)[0] + '.' + x
    ...
    def create_image(self, dvi_fn):
        ...
        output_fn = '%s.%s' % (os.path.splitext(dvi_fn)
                               [0], self.__format.value)
        if self.__format == Format.Png:
            dpi = fontsize2dpi(
                self.__size[1]) if self.__size[1] else self.__size[0]
            return create_png(dvi_fn, output_fn, dpi, self.__background)
        if not self.__size[1]:
            self.__size[1] = 12  # 12 pt
        return create_svg(dvi_fn, output_fn)

    def convert(self, tex_document, base_name):
        """Convert the given TeX document into an image.

        The base name is used to create the required intermediate files
        and the resulting file will be made of the base_name and the
        format-specific file extension. This function returns the
        positioning information used in the CSS style attribute.
        """
        if not isinstance(tex_document, LaTeXDocument):
            raise TypeError(
                ('expected object of type typesetting.LaTeXDocument,' ' got %s')
                % type(tex_document)
            )
        dvi = '%s.dvi' % base_name
        try:
...
def create_png(dvi_fn, output_name, dpi, background):
    ...
    being written to disk. By default, the background of the resulting image is
    transparent, setting any other value will make it use whatever was is set
    in the DVI file.
    ...
    cmd += [
        '--height*',
        '--depth*',
        '--width*',  # print information for embedding
        '-o',
        output_name,
        dvi_fn,
    ]
    ...
    for line in data.split('\n'):
        ...
        if found:
            return dict(zip(['depth', 'height', 'width'], map(float, found.groups())))
