...
class DocumentSerializationException(Exception):
    ...
    formula could not be replaced by a LaTeX command. It provides the following
    attributes:
...
def escape_unicode_maths(formula, replace_alphabeticals=True):
    ...
    It also parses the formula for commands as e.g. \\\text or \\mbox
    and applies text-mode commands within them. This allows the
    conversion of formulas with unicode maths with old-style LaTeX2e,
    which gleetex depends on.
    """
...
def replace_unicode_characters(characters, is_math, replace_alphabeticals=True):
    """Replace all non-ascii characters within the given string with their
    LaTeX equivalent. The boolean is_math indicates, whether text-mode commands
    (like in \\text{}) or the amsmath equivalents should be used. When
    replace_alphabeticals is False, alphabetical characters will not be
    replaced through their LaTeX command when in text mode, so that text
    within.

    \\text{} (and similar) is not garbled. For instance, \\text{für} is
    be replaced by \\text{f\"{u}r} when replace_alphabeticals=True. This
    is useful for the alt attribute of an image, where the reader might
    want to read the normal text as such. This function raises a
    ValueError if a unicode point is not in the table. The first
    argument of the ValueError is the index within the string, where the
    unknown unicode character has been encountered.
    """
    result = []
    for idx, character in enumerate(characters):
        ...
        # string; don't replace alphabeticals if specified
            ...
            if mode == unicode.LaTeXMode.mathmode and mode not in commands:
                result.append('\\text{%s}' %
                              commands[unicode.LaTeXMode.textmode])
            else:
...
class LaTeXDocument:
    ...
    It is intended to contain an equation as main content and properties
    to customize it. Its main purpose is to provide a str method which
    will serialize it to a full LaTeX document.
    """
    ...
    def set_background_color(self, color):
        ...
        The `color` can be either a valid dvips name or a tuple with RGB
        values between 0 and 1. If unset, the image will be transparent.
        """
    ...
    def set_foreground_color(self, color):
        ...
        The `color` can be either a valid dvips name or a tuple with RGB
        values between 0 and 1. If unset, the text will be black.
        """
    ...
    def _format_color_definition(self, which):
        color = getattr(self, '_%s__%s_color' %
...
def increase_readability(formula, replace_nonascii=False):
    ...
    the LaTeX code should be as readable as possible.

    Therefore the formula should not contain unicode characters or
